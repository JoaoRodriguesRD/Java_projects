...
class TesteLocadoraResumido {
	...
	void testeCalcularAluguel() {
		...
		// Confirmando valor do aluguel da moto: (40(diaria) + 4.52(seguro diario)) * 5
		// dias = 222.6
	...
	void testeFaturamentoTotal() {
		...
		locadora.inserir(cli1);
		Date hoje = new Date();
		Date ontem = new Date(hoje.getTime() - 2 * 86400000);
		Date amanha = new Date(hoje.getTime() + 2 * 86400000);

		locadora.registrarAluguel("X-911", hoje, 1, 1234); // Valor do aluguel = 222.6 (moto)
		locadora.registrarDevolucao("X-911");

		assertEquals(44.52, locadora.faturamentoTotal(1, ontem, amanha), 0.01); // Faturamento total de motos
	}
	...
	void testeQuantidadeTotalDeDiarias() {
		...
		locadora.registrarAluguel("X-911", hoje, 1, 1234); // 5 diárias de moto
		locadora.registrarDevolucao("X-911");

		assertEquals(1, locadora.quantidadeTotalDeDiarias(0, ontem, amanha)); // Quantidade de diárias de moto
	}
